...
export async function setupVite(app: Express, server: Server) {
  // Define server options with proper types
...
interface Storage {
  ...
  expireSession(sessionId: number): Promise<void>;
}

export async function setupSessionExpirationCheck(storage: Storage) {
  const checkExpirations = async () => {
    try {
      // Get all sessions that are active
      const sessions = await storage.getAllSessions();
      const activeSessions = sessions.filter((session: Session) => session.is_active);
      ...
      for (const session of activeSessions) {
        ...
        if (currentTime > expiryTime) {
          ...
          const presentStudentIds = attendanceRecords.map((record: AttendanceRecord) => record.user_id);
          const absentStudents = students.filter((student: User) => !presentStudentIds.includes(student.id));
